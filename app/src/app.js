...
client.on('message', async msg => {
    ...
    if (contact.data.state === 0) { // Novo contato
        await levelingController.newLevelingContact(msg, contact).catch(err => console.log(err))
    }
    else { // Contato existente
        if (msg.type === 'buttons_response') {
            const resp = await messageController.getMessageByResponse(msg.body).catch(err => console.log(err))
            if (resp.data.state === contact.data.state) {
                await levelingController.sendNextQuestion(msg, contact).catch(err => console.log(err))
            }
            else if (resp.data.state < contact.data.state) {
                await helpers.sendAlreadyResponse(msg, client).catch(err => console.log(err))
            }
